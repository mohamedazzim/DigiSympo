...
export default function PublicRegistrationFormPage() {
  ...
  const { data: groupedEvents, isLoading: isLoadingEvents } = useQuery<{
  technical: EventWithRounds[];
  non_technical: EventWithRounds[];
}>({
  queryKey: ['/api/events/for-registration-grouped'],
  ...
  // Normalize events regardless of server grouping shape.
  // Some responses may be: flat array, { technical: [...], non_technical: [...] },
  // or other grouped object shapes. Flatten any array values to a single list.
  let rawEvents: EventWithRounds[] = [];
  if (Array.isArray(groupedEvents)) {
    rawEvents = groupedEvents as EventWithRounds[];
  } else if (groupedEvents && typeof groupedEvents === 'object') {
    const _g = groupedEvents as any;
    // Take any array-valued properties and flatten them.
    rawEvents = Object.values(_g)
      .filter((v: any) => Array.isArray(v))
      .reduce((acc: EventWithRounds[], arr: any[]) => acc.concat(arr), []);
  }

  // Derive categories from the normalized list to avoid trusting server grouping
  // Normalize category string variants (e.g. "non-technical", "Non Technical", "nonTechnical")
  const normalizeCategory = (cat: any) => {
    if (!cat) return cat
    const s = String(cat).toLowerCase()
    if (s.includes('non')) return 'non_technical'
    if (s.includes('tech')) return 'technical'
    return s.replace(/[^a-z_]/g, '_')
  }

  rawEvents = rawEvents.map((e) => ({ ...(e as any), category: normalizeCategory((e as any).category) })) as EventWithRounds[];

  const technicalEvents = rawEvents.filter((e) => e.category === 'technical');
  const nonTechnicalEvents = rawEvents.filter((e) => e.category === 'non_technical');

  // Use rawEvents for overlap checks
  ...
  const hasTimeOverlap = (event1Id: string, event2Id: string): boolean => {
    ...
    for (const r1 of e1.rounds) {
      for (const r2 of e2.rounds) {
        // rounds may already be Date objects or strings, normalize to Date
